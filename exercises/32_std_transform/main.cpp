...
std::string tranFunc(int num){
    return std::to_string(2 * num);//×2并且转换为字符串
}
int main(int argc, char **argv) {
    std::vector<int> val{8, 13, 21, 34, 55};
    // TODO: 调用 `std::transform`，将 `v` 中的每个元素乘以 2，并转换为字符串，存入 `ans`
    std::vector<std::string> ans(val.size());//需要初始化形状
    std::transform(val.begin(), val.end(), ans.begin(), tranFunc);
